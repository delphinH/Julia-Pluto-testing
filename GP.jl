...
# ╔═╡ 570a1540-87e2-11ec-3d92-c72eea2822fd
begin
	using Turing, Distributions
	import Turing: Variational
...
# ╔═╡ c4f829c2-4e04-4555-bc09-a7c997199496
md"""
### Gaussian Process
I want to start with the definition of **Gaussian process**.

Definition : A Gaussian process is *a collection of random variables*, any finite number of which have a joint gaussian distribution.

A Gaussian process is completely specified by its mean function and covariance function. We define mean function $\mathcal{m}(x)$ and the covariance function $\mathcal{k}(x, x')$ of a real process $\mathcal{f}(x)$ as

$$\mathcal{m}(x) = \mathbb{E}[f(x)]$$
$\mathcal{k}(x, x') = \mathbb{E}[(f(x) - \mathcal{m}(x))(f(x') - \mathcal{m}(x'))]$
$f(x) \sim \mathcal{GP}(\mathcal{m}(x), \mathcal{k}(x, x'))$

Usually, GP take the mean function to be zero, so the covariance matrix(kernel) is the key ingradient. Moreover, GP has useful property, the marginalization property. That means we have submatrix of the covariance matrix without any changes.(also adding!)

As we mentioned, the kernel is the key componet of GP. Therefore there are several subjects to know about the kernel. But I am not gonna explain yet. If you are interested in that, google *kernel cookbook*.

Gaussian process is the model that can do regression and classification. See the below figure. 


![an image](https://i.imgur.com/PvHBTMg.png)
"""

# ╔═╡ d806f5e5-fa0f-460a-b18d-5f1ff318d77a
url = "https://raw.githubusercontent.com/delphinH/imagetoy/main/dataset/Fish.csv"

# ╔═╡ 0d6d125a-7e32-460f-8e4c-9e6cfa17b5d8
fish = CSV.read(HTTP.get(url).body, DataFrame)

# ╔═╡ 05b2b441-1b5a-4235-baf7-e33ec9e39b10
@df fish scatter(:Length1, :Weight)

# ╔═╡ 0d82bd91-28f6-43a0-ba35-506d0b123407
begin
	xs = fish[:, :Length1]
	ys = fish[:, :Weight]
end

# ╔═╡ d26eac35-b1b8-4d76-a240-5ccf962e14fd
size(xs)

# ╔═╡ 1b146cd3-a844-475c-bb11-f3251b255fb0
...
# ╔═╡ 0d95556d-119e-4457-9180-45664808dcfe
x = 1:60

# ╔═╡ 610eb4f0-d6b3-4b9e-94ad-4b2f406c7f58
mu = zeros(length(x))

# ╔═╡ a6a7edb1-c371-4e59-9b27-00a7e898a2be
cov = EQkernel(x, x, 1, 1)

# ╔═╡ 96af16c8-cf13-42e0-8054-1fe817ce0431
prior = rand(MvNormal(mu, Matrix(Hermitian(cov + 0.001*I))), 3)

# ╔═╡ 37c1c0a2-0450-41cc-abea-3aa8c32c813d
plot(prior[:, 1:3])

# ╔═╡ 9ca722fb-060c-4be3-9ceb-666b37289138
function post(x_n, x_t, y_t, l, σ, sigma_y)
	K = EQkernel(x_t, x_t, l, σ) + sigma_y^2 * I
	K_s = EQkernel(x_t, x_n, l, σ)
	K_ss = EQkernel(x_n, x_n, l, σ)

	mu_n = transpose(K_s) * inv(K) * y_t

	cov_n = K_ss - transpose(K_s) * inv(K) * K_s
	return mu_n, cov_n, K_s, K_ss
end

# ╔═╡ c1218fd1-01bd-4b36-91a0-9ae074063c3a
X_new = reshape(collect(range(-3.5, 3.5, length=100)), 100, 1)

# ╔═╡ 43d6fda7-23f7-477b-9c49-7fa28554fc6c
mu_n, cov_n, K_s, K_ss = post(x, xs, ys, 2, 1, 0.5)

# ╔═╡ 5ef8e96c-3d78-452f-8791-f7f2ba915f67
cov_n

# ╔═╡ 8cee8595-34f0-4798-99b4-0b6307bbcd3b
posterior = rand(MvNormal(mu_n, Matrix(Hermitian(cov_n + 0.001*I))), 3)

# ╔═╡ c976e59b-a691-4171-886b-7d7b611a9cc8
begin
	scatter(xs, ys, legend=false)
	plot!(posterior[:, 1:3])
end

# ╔═╡ 7e91e628-0d44-471c-bb95-d425ea4499f6
@model gptest(x, y) = begin
	#Prior
	σ ~ LogNormal(1.0, 0.5)
	l ~ LogNormal(1.0, 1.0)
	sigma_y ~ LogNormal(0.0, 1.0)
	
	#covariance function
	kernel = EQkernel(x, x, l, σ)
    
	#add noise
    y ~ MvNormal(kernel + sigma_y * I)
end

# ╔═╡ fe9da0ce-6282-418f-a6f0-9ef566458e5b
model = gptest(xs, ys)

# ╔═╡ e0ee7e7c-0d43-4bc8-bea8-5c23f4dd9ea7
q0 = Variational.meanfield(model)

# ╔═╡ 6d9782ce-cb04-4ef6-9e8c-624cef812f23
advi = ADVI(10, 2000)

# ╔═╡ 1039e3ad-a949-48a4-842b-2e9e2cbb8322
q = vi(model, advi, q0; optimizer = Flux.ADAM())

# ╔═╡ ade10ceb-03f0-4ae3-8ca2-f5bcc08699b7
z = rand(q, 5000);

# ╔═╡ 402d5b3f-7443-453d-bc9b-ee4b8c47f0d1
begin
	_, sym2range = bijector(model, Val(true));
	sym2range
end

# ╔═╡ 9946c2b6-5991-4ed0-9766-b7698e9be019
...
# ╔═╡ ceb6a9a9-407e-4117-9240-11b15e38d9dc
plot_variational_marginals(z, sym2range)

# ╔═╡ 200e262a-63ee-4538-9225-5a6beca7183c
avg = vec(mean(z; dims = 2))

# ╔═╡ 2b5e5118-f351-4a68-8045-ad322945a877
avg[union(sym2range[:l]...)]

# ╔═╡ 30107419-923c-4705-89b4-94779abdf6e3
avg[union(sym2range[:σ]...)]

# ╔═╡ 4c0ac354-1f98-46f2-91b0-538db3e761f0
avg[union(sym2range[:sigma_y]...)]

# ╔═╡ f67b43ef-cdaa-4eeb-81c0-4634e6b4578e
md"""
### Reference
- Gaussian Processes for Machine Learning by C. E. Rasmussen and C. K. I. Williams
- Zoubin Ghahramani NIPS 2016 slide
"""
...
git-tree-sha1 = "69cedd6981e20a30b5032c3551d4d7809fe51f5d"
...
git-tree-sha1 = "8a5907515efa0c471a275f087804a4e063a1d3e3"
